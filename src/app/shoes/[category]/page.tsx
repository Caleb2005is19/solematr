...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const categoryDetails = await getCategoryDetails('shoes', params.category);
...
export default async function ShoeCategoryPage({ params, searchParams }: Props) {
  ...
  const { title, description } = await getCategoryDetails('shoes', category);

  if (title.includes('All ')) {
      notFound();
  }

  const shoes = await getShoes({ type: 'Shoes', category, ...searchParams });
  
  const allShoesInCategory = await getShoes({ type: 'Shoes', category });
        ...
        <div className="space-y-4">
            <Breadcrumbs
                items={[
                    ...
                    { label: capitalize(category), href: `/shoes/${category}`, isActive: true },
                ]}
            />
